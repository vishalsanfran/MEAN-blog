...
describe('Articles Controller Unit Tests:', function() {
  ...
  describe('Testing the GET methods', function() {
    ...
    //using superTest assertion library to issue HTTP GET at the endpoint    
      request(app).get('/api/articles/')
        .set('Accept', 'application/json')
        .expect('Content-Type', /json/)
        .expect(200)    //examine http content-type, response code
        .end(function(err, res) { // finished superTest
        //Should.js assertion expressions to test the response body    
  ...
  afterEach(function(done) {    //cleaning up..
    Article.remove().exec();
    User.remove().exec();
    done();
  });
